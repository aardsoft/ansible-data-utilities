class FilterModule(object):
    def filters(self):
        ''' boolconv takes a bool in the forms yes/no, true/false, on/off
            enabled/disabled or 0/1 and converts it to a string representation
            based on its additional argument:  onoff, ONOFF, truefalse,
            TrueFalse, TRUEFALSE, yesno, YesNo, YESNO, enableddisabled or int.
    ...
    def boolconv(self, value, bool_type="truefalse"):
        if (str(value).lower()=="true"):
            ...
            ivalue=False
        elif (str(value).lower()=="enabled"):
            ivalue=True
        elif (str(value).lower()=="disabled"):
            ivalue=False
        ...
        if (bool_type=="YESNO"):
            return ("NO","YES")[ivalue]
        if (bool_type=="enableddisabled"):
            return ("disabled","enabled")[ivalue]
        if (bool_type=="int"):
            return ("0","1")[ivalue]
